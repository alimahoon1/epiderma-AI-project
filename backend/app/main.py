...
def answer_with_rag(user_input):
    ...
    allowed_keywords = [
        "acne", "pimple", "blackhead", "whitehead", "papule", "pustule",
        "nodule", "dark spot", "skin", "routine", "treatment", "face",
        "hyperpigmentation"
    ]

    if not any(k in user_input.lower() for k in allowed_keywords):
    ...
    # Create answer
    reply = f"""
Epiderma here! Based on your question:
---------
{context}
---------
Let me know if you'd like advice or a routine!
"""
    return reply.strip()

@app.post("/chat", response_model=ChatResponse)
async def chat(text: str = Form(...)):
    reply = answer_with_rag(text)
    return {"reply": reply}
